...
from email_template_check import __version__
from email_template_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from email_template_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='email-template-check',
        description=(
    ...
    parser.add_argument("input", nargs="?", type=Path, help='email template')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"email-template-check: error: {exc}", file=sys.stderr)
        return 1
